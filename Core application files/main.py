#!/usr/bin/env python3
"""
Main entry point for the OMNI News Pipeline with Twitter Webhook Integration
...
def setup_database():
    ...
    try:
        ...
        from models.models import Entity, Source, Tweet, TwitterWebhook
        ...
        db = SessionLocal()
        try:
            # Add Sean McVay entity with Twitter tracking
            ...
            if not entity:
                entity = Entity(
                    ...
                    is_active=True,
                    twitter_username="SeanMcVay",
                    is_twitter_tracked=False
                )
                db.add(entity)
                print("✓ Added Sean McVay entity with Twitter tracking")
...
def start_webhook_server():
    ...
    try:
        from webhook_endpoints import webhook_app
...
def start_twitter_worker():
    """Start Celery worker for Twitter tasks"""
    print("Starting Twitter Celery worker...")
    
    try:
        import subprocess
        import sys
        
        # Start worker with Twitter queue
        cmd = [
            sys.executable, "-m", "celery", "worker",
            "--app=tasks.celery_app:celery_app",
            "--loglevel=info",
        ...
        print("Starting Twitter worker with command:", " ".join(cmd))
        subprocess.run(cmd)
        
    except KeyboardInterrupt:
        print("\nTwitter worker stopped by user")
    except Exception as e:
        print(f"✗ Error starting Twitter worker: {e}")
        sys.exit(1)
...
def run_tests():
    ...
    try:
        ...
        print("✓ Twitter webhook service initialized successfully")
...
def test_twitter_webhook():
    """Test the Twitter webhook system"""
    print("Testing Twitter webhook system...")
    ...
    try:
        ...
        # Test health check
        print("✓ Twitter webhook system ready")
        
    except Exception as e:
        print(f"✗ Twitter webhook test failed: {e}")
        sys.exit(1)
...
def show_status():
    ...
    try:
        from models.database import SessionLocal
        from models.models import Entity, Article, Source, Tweet, TwitterWebhook
        ...
        print(f"Twitter Webhooks: {webhook_count}")
        ...
        # Recent tweets
        recent_tweets = db.query(Tweet).filter(Tweet.created_at >= week_ago).count()
        print(f"Recent tweets (7 days): {recent_tweets}")
        ...
        print("Article sentiment distribution:")
        for sentiment, count in sentiment_counts.items():
            print(f"  {sentiment}: {count}")
        
        # Tweet sentiment distribution
        tweet_sentiment_stats = db.query(Tweet.sentiment_label).filter(
            Tweet.sentiment_label.isnot(None)
        ).all()
        
        tweet_sentiment_counts = {}
        for label in tweet_sentiment_stats:
            label = label[0] if label[0] else 'neutral'
            tweet_sentiment_counts[label] = tweet_sentiment_counts.get(label, 0) + 1
        
        print("Tweet sentiment distribution:")
        for sentiment, count in tweet_sentiment_counts.items():
            print(f"  {sentiment}: {count}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OMNI News Pipeline with Twitter Webhooks")
    parser.add_argument('command', 
                       choices=['setup', 'scrape', 'worker', 'beat', 'dashboard', 
                               'streamlit', 'webhook', 'twitter-worker', 'test', 'status'],
                       help='Command to run')
    
    args = parser.parse_args()
    
    print("OMNI News Pipeline with Twitter Webhooks")
    print("=" * 50)
    ...
    elif args.command == 'twitter-worker':
        start_twitter_worker()
